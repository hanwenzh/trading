...
namespace Trade.Biz
{
    public class DataBiz
    {
        ...
        public static void OrderMerge(ref IEnumerable<Order> lst)
        {
            ...
            {
                int count = x.Sum(i => string.IsNullOrWhiteSpace(i.order_no) ? 0 : i.count);
                return new Order()
                {
        ...
        public static void PositionMerge(ref IEnumerable<Position> lst)
        {
            lst = lst.GroupBy(p => new { p.code, p.name, p.unit_id, p.unit_name }).Select(x =>
            {
                ...
                return new Position()
                {
                    ...
                    price_cost = Math.Round(count == 0 ? x.Average(i => i.price_cost) : x.Sum(i => i.price_cost * i.count) / count, 3),
                    price_cost_today_buy = Math.Round(count_today_buy == 0 ? x.Average(i => i.price_cost_today_buy) : x.Sum(i => i.price_cost_today_buy * i.count_today_buy) / count_today_buy, 3),
                    price_cost_today_sell = Math.Round(count_today_sell == 0 ? x.Average(i => i.price_cost_today_sell) : x.Sum(i => i.price_cost_today_sell * i.count_today_sell) / count_today_sell, 3),
