...
namespace RedisSrv.Helper
{
    ...
    internal class RedisHelper
    {
        ...
        internal RedisHelper(int database)
        {
            ...
            {
                _connMultiplexer = ConnectionMultiplexer.Connect(string.Format("{0},password={1},allowAdmin=true,syncTimeout=5000", IPPort, Password));
            }
            _db = _connMultiplexer.GetDatabase(database);
        }
        #endregion 构造函数

        internal void FlushDatabase(List<int> databases)
        {

            IServer _srv = _connMultiplexer.GetServer(IPPort);
            databases.ForEach(db =>
            {
                _srv.FlushDatabase(db);
            });
        }
        #region String 操作
        ...
        internal double HashIncrement(string redisKey, string hashField, float delta)
        {

            return _db.HashIncrement(redisKey, hashField, delta);
        }

        internal long HashIncrement(string redisKey, string hashField, int delta)
        {
