...
namespace JY
{
    public class JYService
    {
        ...
        {
            string cmd = string.Format("9002|{0}", order_nos);
            cmd_order.Add(cmd);
        }
        ...
        private void QueryDeal_Result(string response)
        {
            ...
            if (datas.Length > 0)
            {
                foreach (string data in datas)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(deal.deal_no) || !orders.ContainsKey(deal.order_no))
                        continue;
