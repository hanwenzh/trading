...
namespace MySQLSrv
{
    ...
    {
        public static List<AccountGroup> List(int user_id)
        {
            ...
            groups.ForEach(g =>
            {
                g.items = items.Where(i => i.account_group_id == g.id).DistinctBy(i => i.account_id).ToList();
            });
            return groups;
        }

        public static List<Base> List4Filter(int user_id)
        {
            string sql = "SELECT * FROM account_group";
            if (user_id > 0)
                sql += " WHERE FIND_IN_SET(created_by, f_get_user_children(" + user_id + "))";
            DataTable dt = MySQLHelper.GetDataTable(sql, null);
            List<Base> groups = DAHelper.GetListByDataTable<Base>(dt);
            return groups;
        }
