...
namespace AutoUpgrade.DB.Helper
{
        ...
        internal static T GetModelByDataRow<T>(DataRow dr) where T : new()
        {
            ...
            foreach (PropertyInfo pInfo in model.GetType().GetProperties())
            {
                ...
                if(pInfo.PropertyType == typeof(Int32))
                    ...
                    pInfo.SetValue(model, (string.IsNullOrEmpty(val) ? 0 : Single.Parse(val)), null);
                else if (pInfo.PropertyType == typeof(DateTime))
                    pInfo.SetValue(model, (string.IsNullOrEmpty(val) ? DateTime.MinValue : DateTime.Parse(val)), null);
                else if (pInfo.PropertyType == typeof(decimal))
                    pInfo.SetValue(model, (string.IsNullOrEmpty(val) ? 0 : decimal.Parse(val)), null);
                else
