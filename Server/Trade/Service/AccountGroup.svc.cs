...
namespace Trade.Service
{
    ...
    public class AccountGroup : Service, IAccountGroup
    {
        public Result<List<Model.DB.AccountGroup>> List()
        {
            ...
            List<Model.DB.AccountGroup> lst = AccountGroupDA.List(role == UserRoleEnum.Administrator ? 0 : user_id);
            return Result(lst);
        }
        ...
        {
            UserRoleEnum role = (UserRoleEnum)int.Parse(UserRA.Get(user_id.ToString(), "role"));
            List<Base> lst = AccountGroupDA.List4Filter(role == UserRoleEnum.Administrator ? 0 : user_id);
            return Result(lst);
        }
